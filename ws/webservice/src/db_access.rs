...
(pool: &PgPool, teacher_id: i32) -> Result<Vec<Course>, MyError> {
    let rows = sqlx::query!(
        ...
        FROM courses
        WHERE teacher_id = $1"#, // Fixed the WHERE clause
        teacher_id
    )
    .fetch_all(pool)
    .await?;
...
pub async fn get_course_details_db(
    ...
    let row = sqlx::query!(
        r#"SELECT id, teacher_id, name, time
        FROM courses // Changed to "courses" table name
        WHERE teacher_id = $1 and id = $2"#,
        teacher_id,
        course_id
    )
    .fetch_one(pool) // Added fetch_one here
    .await;
...
pub async fn post_new_course_db(
    ...
    let row = sqlx::query!(
        r#"INSERT INTO courses (teacher_id, name) // Removed "id" from INSERT
        VALUES ($1,$2)
